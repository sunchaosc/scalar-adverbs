...
setwd('/Users/chaosun/Dropbox/My Experiments/scalar-adverbs/scalar-adverbs/experiments/01_incremental/pilot/mturk')
theme_set(theme_bw())

df = read.csv("incremental-trials.csv", header = TRUE)
demo = read.csv("incremental-subject_information.csv", header = TRUE)
# df = df[df$workerid!=2,]
#formatting
...
toplot =  df %>%
  ...
  select(workerid,condition,modal,click1,click2,click3,click4,target,competitor,instruction3) %>%
  mutate(ID = row_number()) %>%
  gather(click_number,location,click1:click4) %>%
  mutate(target=ifelse(location==target,1,0)) %>%
  mutate(competitor=ifelse(location==competitor,1,0)) %>%
  group_by(condition,modal,click_number) %>%
