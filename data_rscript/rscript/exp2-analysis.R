## ---- setup, include=FALSE--------------------------------------------------------------------------
pacman::p_load(mosaic,  tidyverse, bootstrap, ggplot2, lme4, lmerTest, buildmer)
...
df <- read.csv("exp2_cleaned_data.csv", header = TRUE)

exp <- df[df$ExpFiller=="Exp",]

exp$polarity <- ifelse(exp$condition %in% c("all", "some"), "pos", "neg")
exp$type <- ifelse(exp$condition %in% c("some", "not all"), "prag", "sem")

cols <- c('workerid', 'list', 'displayID', 'polarity', 'type','condition')
exp[cols] <- lapply(exp[cols], factor)

exp = exp %>% select(list, workerid,ExpFiller,displayID,condition,polarity,type, 
...
sexp$condition <- factor(exp$condition)
...
toplot = exp %>%
  ...
  summarize(m_target=mean(target_click),
            ...
            ci_low_competitor=ci.low(competitor_click),ci_high_competitor=ci.high(competitor_click)) %>%
  ...
  mutate(click_number=fct_recode(click_number,baseline="click1",object="click2",
                                 quantifier="click3",color="click4"))%>% 
  select(-c(ci_low_target, ci_high_target, ci_low_competitor, ci_high_competitor, 
            location ))

toplot$condition <- factor(toplot$condition, 
                           levels = c("some", "all", "not all", "none"))
condition.labs <- c("some", "all", "not all", "none")
names(condition.labs) <-c("some", "all", "not all", "none")
...
proportions
ggsave(filename = "prop-exp2.jpeg", proportions, width = 9, height = 4, dpi = 300, units = "in", device='jpeg')
...
## ---------------------------------------------------------------------------------------------------
buildmer(target_click ~ type*polarity + (type*polarity|workerid) + (type*polarity|displayID),
         data=d_base, family="binomial",
         buildmerControl = buildmerControl(include = ~ type*polarity +
                                             (1|workerid) + (1|displayID),
                                           list(direction = "order")))

m.base = glmer(target_click ~ type*polarity + (1|displayID),
...
m.object = glmer(formula = target_click ~ 1 + type + polarity + type:polarity+ (1|displayID), 
...
buildmer(target_click ~ type*polarity 
         + (type*polarity|workerid) + (type*polarity|displayID),
...
m.adverb = glmer(target_click ~ 1 + type + polarity + type:polarity 
                 + (1 | workerid)+ (1|displayID), 
                 family="binomial", data=d_adverb) 
summary(m.adverb)

summary(glmer(target_click ~ type + (1|workerid)+ (1|displayID),
              family="binomial", data=d_adverb[d_adverb$polarity=="pos",]))

summary(glmer(target_click ~ type + (1|workerid) ,                 
              family="binomial",data=d_adverb[d_adverb$polarity=="neg",]))

summary(glmer(target_click ~ polarity + (1|workerid) + (1|displayID),                 
              family="binomial", data=d_adverb[d_adverb$type=="prag",])) 

summary(glmer(target_click ~ polarity + (1|workerid),                 
